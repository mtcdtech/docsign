# Walkthrough - Registrations Rename, PCO Integration & Light/Dark Brand Logos (v0.14.0 - v0.14.1)

This walkthrough documents the design and verification status of the completed features under the v0.14.1 release.
...
### C. Planning Center Online (PCO) Sync Details Dashboard & Inline Cards
- **Files**:
...
- **Modifications**:
  - Added `getPcoRegistrationAttendees` and `getPcoQuestions` helpers to [pco.ts](file:///Users/benny2168/Antigravity/docsign/src/lib/pco.ts).
  - Developed GET endpoint under `/api/admin/registrations/[id]/pco` that queries attendees registered for the connected PCO Signup ID. Cross-references database records to compute signer status ("Completed", "Partial", "Not Started").
...
### D. App & Organization Light/Dark Brand Logos
- **Files**:
  - [SettingsForm.tsx](file:///Users/benny2168/Antigravity/docsign/src/app/admin/settings/SettingsForm.tsx)
  - [route.ts (settings)](file:///Users/benny2168/Antigravity/docsign/src/app/api/admin/settings/route.ts)
  - [route.ts (organization patch)](file:///Users/benny2168/Antigravity/docsign/src/app/api/admin/organizations/%5Bid%5D/route.ts)
- **Modifications**:
  - Expanded Settings to store `portal_logo_light` and `portal_logo_dark` properties.
  - Added drag-and-drop / select uploader interfaces for App Logo (Light Mode) and App Logo (Dark Mode) in the Settings tab.
  - Created a new tab "Organization Branding" under Settings displaying synced organizations list, with independent file upload inputs to save base64 Light and Dark mode logos for each.
  - Modified client-facing wizard and forms header to dynamically display organization logos (prioritized) or portal logo, using `@media (prefers-color-scheme)` parameters to select light vs. dark mode assets instantly without hydration lag.

### E. Branded Email Logos Integration
- **File**: [route.ts (sign handler)](file:///Users/benny2168/Antigravity/docsign/src/app/api/sign/%5Bid%5D/route.ts)
- **Modifications**:
  - Modified email builder template `getEmailHtml` to receive and render both the global App Logo and the specific Organization Logo side-by-side inside the dark header card block using high-compatibility HTML tables.

### F. System Audit Log Text Contrast Fix
